import axios from "axios";
import React, { useEffect, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { addFavMovie, removeFavMovie } from "../redux/actions/actions";
import ModalComponent from "../components/Modal";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faSearch, faHeart } from "@fortawesome/free-solid-svg-icons";
import NoData from "../components/NoData";

const Icon = () => {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="white">
            <path d="M244 84L255.1 96L267.1 84.02C300.6 51.37 347 36.51 392.6 44.1C461.5 55.58 512 115.2 512 185.1V190.9C512 232.4 494.8 272.1 464.4 300.4L283.7 469.1C276.2 476.1 266.3 480 256 480C245.7 480 235.8 476.1 228.3 469.1L47.59 300.4C17.23 272.1 0 232.4 0 190.9V185.1C0 115.2 50.52 55.58 119.4 44.1C164.1 36.51 211.4 51.37 244 84C243.1 84 244 84.01 244 84L244 84zM255.1 163.9L210.1 117.1C188.4 96.28 157.6 86.4 127.3 91.44C81.55 99.07 48 138.7 48 185.1V190.9C48 219.1 59.71 246.1 80.34 265.3L256 429.3L431.7 265.3C452.3 246.1 464 219.1 464 190.9V185.1C464 138.7 430.4 99.07 384.7 91.44C354.4 86.4 323.6 96.28 301.9 117.1L255.1 163.9z"/>
        </svg>
    )
}

function MovieList() {
    const favMovieState = useSelector((state) => state.favMovie)
    const dispatch = useDispatch();
    const apiKey = 'b09bee0d';

    const [search, setSearch] = useState();
    const [modal, setModal] = useState(false);
    const [detail, setDetail] = useState();
    const [color, setColor] = useState('#fff');
    const [list, setList] = useState();

    const fetchMovies = async (search) => {
        try {
            const res = await axios.get(`http://www.omdbapi.com/?s=${search}&apikey=${apiKey}`)
            
            const movie = [];
            if (res?.data?.Search?.length) {
                for (const item of res.data.Search) {
                    const same = favMovieState.some(el => el.imdbID === item.imdbID);
                    movie.push({
                        Poster: item?.Poster,
                        Title: item?.Title,
                        Type: item?.Type,
                        Year: item?.Year,
                        imdbID: item?.imdbID,
                        like: same ? true : false
                    })
                }
            }
    
            setList(movie);
        } catch (e) {
            console.log(e);
        }
    }

    const handleLike = (item, idx) => {
        const arr = list.map((el, i) => {
            if (i === idx) {
                el.like = !el.like
            }
            return {...el}
        })

        setList(arr);
        dispatch(addFavMovie(item));
    }

    const handleDislike = (item, idx) => {
        const arr = list.map((el, i) => {
            if (i === idx) {
                el.like = !el.like
            }
            return {...el}
        })

        setList(arr);
        dispatch(removeFavMovie(item));
    }

    useEffect(() => {
        setSearch('')
    }, []);

    return (
        <>
            <div className="mt-3 mx-auto">
                <div className="has-search">
                    <FontAwesomeIcon className='search' icon={faSearch} color={color} flip />
                    <input
                        className="form-control"
                        type="search"
                        placeholder="Search movie title here"
                        onFocus={() => setColor('#000')}
                        onBlur={() => setColor('#fff')}
                        onChange={(event) => {
                            setTimeout(() => {
                                setSearch(event.target.value)
                                fetchMovies(event.target.value);
                            }, 1000)
                        }}
                    />
                </div>

                <div>
                { list?.length ? 
                    <table className="table mt-4">
                        <thead>
                            <tr>
                                <th scope="col">TITLE</th>
                                <th scope="col">YEAR</th>
                                <th scope="col">IMDB ID</th>
                                <th scope="col"></th>
                            </tr>
                        </thead>
                        <tbody>
                            {list.map((item, i) => {
                                return (
                                <tr key={i} className="table-row">
                                    <th style={{ cursor: 'pointer' }} onClick={() => {setModal(true); setDetail(item)}}>{item.Title}</th>
                                    <td >{item.Year}</td>
                                    <td >{item.imdbID}</td>
                                    <td >
                                        {item.like ? 
                                            <div style={{ width: 25, height: 26, display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer' }} onClick={ () => { handleDislike(item, i) }}> <FontAwesomeIcon icon={faHeart} color="white" size="xl" /> </div> :
                                            <div style={{ width: 25, height: 26, display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer' }} onClick={ () => { handleLike(item, i) }}> <Icon /> </div>
                                        }
                                    </td>
                                </tr>
                                )
                            })}
                        </tbody>
                    </table>
                    :
                    (search && <NoData />)
                }
                </div>
                {modal && <ModalComponent data={detail} close={setModal}/>}
            </div>
        </>
    );
}

export default MovieList;
